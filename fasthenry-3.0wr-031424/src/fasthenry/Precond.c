...
void indPrecond(ssystem *sys, SYS *indsys, double w)
{
  ...
  /* SRW -- added timing and user feedback. */
  int trips, ntrips, ntov10;
  double dtime;
  double A, A0=0.0, B, B0=0.0;
  starttimer(&dtime);
  ...
  if (indsys->precond_type == SPARSE && (indsys->opts->dumpMats & DUMP_Ls)) {
    ...
    if ( (fp_ls = fopen(outfname,"w")) == NULL) {
  ...
  if (indsys->precond_type == SPARSE) {
    /* SRW
     * If all sigma's are zero, then we have pure superconductors and the
     * matrix is purely imaginary.  We can solve as real, saving time.
     */
    int has_sigma = 0;
    SEGMENT *sg;
    for (sg = indsys->segment; sg; sg = sg->next) {
      if (sg->sigma != 0.0) {
        has_sigma = 1;
        break;
      }
    ...
    if (!has_sigma) {
      matSetPureSC(Matrix, 1);
      fprintf(stdout, "All segments are lossless superconductors.\n");
    }
  }
#endif
#endif
...
/*    if (indsys->precond_type == LOC && ) {*/
    ...
    if (meshsize > meshmax) {
      /* SRW -- size increment changed to 64 from 10, added free calls. */
      int newsize = meshsize + 64;

      if (meshnum)
        free(meshnum);
      CALLOC(meshnum, newsize, int, ON, IND);
      ...
      CALLOC(meshmat, newsize, CX*, ON, IND);
      for(i = 0; i < newsize; i++) 
        CALLOC(meshmat[i], newsize, CX, ON, IND);

      if (is_dup)
        free(is_dup);
      CALLOC(is_dup, newsize, DUPS, ON, IND);
    ...
    for(i = 0; i < the_size; i++) {
      ...
      mels = (MELEMENT*)malloc(msz*sizeof(MELEMENT));
      msz = 0;
      for(mtrani=Mtrans[fillist[i]]; mtrani!=NULL;mtrani=mtrani->mnext) {
        if ((filcount[mtrani->filindex] > 0 && !posdef)
            || (filcount2[mtrani->filindex] > 0 && posdef))
        ...
        if (mat[i][j] != 0.0) {
          for(mtranj=Mtrans[fillist[j]]; mtranj != NULL; mtranj = mtranj->mnext) {
            if ((filcount[mtranj->filindex] > 0 && !posdef)
              ...
              double t1 = w*mat[i][j]*mtranj->sign;
              double t2 = R[fillist[i]]*mtranj->sign;
              mcol = indx[mtranj->filindex];
              for (k = 0; k < msz; k++) {
                mrow = indx[mels[k].filindex];
                meshmat[mrow][mcol].imag += mels[k].sign*t1;
                if (i == j)
                  meshmat[mrow][mcol].real += mels[k].sign*t2;
              }
            }
          }
        }
    ...
    if (indsys->precond_type == SPARSE) {
       ...
       * matrices, uses batch insertion.
       *
       */
      matData *data = (matData*)malloc(meshsize*sizeof(matData));
      for(i = 0; i < meshsize; i++) {
        int cnt = 0;
        ...
        for(j = 0; j < meshsize; j++) {
          if (mati[j].real != 0.0 || mati[j].imag != 0.0) {
            data[cnt].row = meshnum[j] + 1;
            data[cnt].real = mati[j].real;
            data[cnt].imag = mati[j].imag;
            cnt++;
          }
        }
        matSetRowElements(Matrix, realmcol+1, data, cnt);
      }
      free(data);
#else
      for(i = 0; i < meshsize; i++) {
        ...
        for(j = 0; j < meshsize; j++) {
          if ( (meshmat[i][j].real != 0.0 || meshmat[i][j].imag != 0.0) ) {
            realmcol = meshnum[j];
            (elem = (CX *)matGetElement(Matrix,realmrow+1,realmcol+1))->real 
                             += meshmat[i][j].real;
...
/*  bigmeshPre(sys, indsys, w);  */
  ...
  /* SRW -- some cleanup */
  if(filcount) free(filcount), filcount = 0;
  if(filcount2) free(filcount2), filcount2 = 0;
  if(is_partial) free(is_partial), is_partial = 0;
  if(maxfilcount) free(maxfilcount), maxfilcount = 0;
  if(indx) free(indx), indx = 0;
  if(findx) free(findx), findx = 0;
  if(is_in_nc) free(is_in_nc), is_in_nc = 0;

  if (meshnum) free(meshnum), meshnum=0;
  if (meshmat) {
    for(i = 0; i < meshmax; i++) 
      free(meshmat[i]);
    free(meshmat);
  ...
  if (is_dup) free(is_dup), is_dup = 0;
  meshmax = 0;

  printf("Block A runtime=%g, block B runtime=%g\n", A0, B0);

  /* SRW */
  stoptimer(&dtime);
  {
    unsigned int sz, nm, rc;
    matStats(Matrix, &sz, &nm, &rc);
    printf("\nPrecond build time %g, matsize %u, nonzero %u recalls %u\n",
      dtime, sz, nm, rc);
  }
}
