...
SPATH *old_path_through_gp(NODES *nodein, NODES *nodeout, GROUNDPLANE *plane)
{
  ...
  (void)find_nearest_gpnode(nodein->x, nodein->y, nodein->z, plane, &bc0, &bc1);
  (void)find_nearest_gpnode(nodeout->x, nodeout->y, nodeout->z, plane, &ec0, &ec1);
