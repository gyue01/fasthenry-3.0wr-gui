...
void mulMatDirect(ssystem *sys)
{
  cube *nextc, *nextnbr;
  int i, nummats, **temp;
  /* SRW -- This function can take a while, add some user feedback. */
  int tmats = 0, mcnt = 0, tov10;;
/*
...
*/
    ...
    if (tmats >= 1000) {
        if (mcnt == 1)
            printf("Multiplying %d matrices...", tmats);
        else if (mcnt % tov10 == 0) {
            printf(" %.1f", (10.0*mcnt)/tov10);
            if (mcnt == 10*tov10)
                printf("\n");
            fflush(stdout);
        }
