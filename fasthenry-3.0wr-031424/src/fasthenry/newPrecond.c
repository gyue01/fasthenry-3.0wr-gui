...
/* double w;  frequency */
{
  ...
  for(i = 0; i < num_mesh; i++) {
    for(j = i; j < num_mesh; j++) {
      ...
      if (used == 1) {
        (elem1 = (CX *)matGetElement(Matrix,i+1,j+1))->real
          = (elem2 = (CX *)matGetElement(Matrix,j+1,i+1))->real = rtempsum;
  ...
  if (indsys->opts->dumpMats & PRE) {
    ...
    if (matFileMatrix(Matrix, outfname, "Precond", 0, 1, 1) == 0)
...
/*  I thought using real matrices might help, but not really in the end.
  if (!indsys->dont_form_Z)
    indsys->sparMatrix = (char *)matCreate(indsys->num_mesh, 1, &err);
  else
    indsys->sparMatrix = (char *)matCreate(indsys->num_mesh, 0, &err);
*/
      ...
      for(j = 0; j < num_fils; j++) {
        ...
        for(mtranj=Mtrans[filj]; mtranj != NULL; mtranj = mtranj->mnext) {
          for(mtrani=Mtrans[fili]; mtrani != NULL; mtrani = mtrani->mnext) {
            ...
            CX *elem = (CX*)matGetElement(Matrix,
              mtrani->filindex+1, mtranj->filindex+1);
            elem->imag += w*mtrani->sign*mat[i][j]*mtranj->sign;
            if (i == j)
              elem->real += mtrani->sign*R[fili]*mtranj->sign;
          }
        }
      }
    }
    ...
    for(i = 0; i < num_fils; i++) {
      ...
      for(mtranj=Mtrans[filnum]; mtranj != NULL; mtranj = mtranj->mnext) {
        for(mtrani=Mtrans[filnum]; mtrani != NULL; mtrani = mtrani->mnext) {
            CX *elem = (CX*)matGetElement(Matrix,
              mtrani->filindex+1, mtranj->filindex+1);
            elem->imag += w*mtrani->sign*val*mtranj->sign;
...
void fill_diagR(SYS *indsys)
{
  ...
  for(seg = indsys->segment; seg != NULL; seg = seg->next) {
    ...
    for(i = 0; i < num_fils; i++) {
      ...
      for(mtranj=Mtrans[filnum]; mtranj != NULL; mtranj = mtranj->mnext) {
          for(mtrani=Mtrans[filnum]; mtrani != NULL; mtrani = mtrani->mnext) {
            *(matGetElement(Matrix,mtrani->filindex+1,
                                        mtranj->filindex+1))
