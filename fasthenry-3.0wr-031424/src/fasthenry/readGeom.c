...
#if SUPERCON == ON
    else if (sscanf(line," tau = %lf%n",&dumb,&skip) == 1) {
      defaults.istau = 1;
      defaults.tau = dumb;
      line += skip;
    }
...
SEGMENT *makeseg(char *name, NODES *node0, NODES *node1, double height,
    ...
    /* double sigma;     conductivity                                         */
#if SUPERCON == ON
    /* double tau;       Drude relaxation time                                */
...
int addgroundplane(char *line, SYS *indsys, GROUNDPLANE **retplane)
{
  ...
  int xred = 0, yred = 0, zred = 0, segheiread = 0;
  int segread1 = 0, segread2 = 0, filenameread = 0;
  int skip;
