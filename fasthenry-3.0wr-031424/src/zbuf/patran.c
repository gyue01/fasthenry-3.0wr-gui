...
void input(FILE *stream, char *line, int surf_type, double *trans_vector)
{
  ...
  while (!END) {
    ...
    else (void)fscanf(stream,"%d %d %d %d %d %d %d %d %d", 
                &type_number, &ID, &IV, &KC, &N1, &N2, &N3, &N4, &N5);
...
void node_data(FILE *stream, double *trans_vector)
{
  ...
  (void)fscanf(stream,"%lf %lf %lf",tmp_coord,tmp_coord+1,tmp_coord+2);  
  waste_line(1,stream);
...
void element_data(FILE *stream)
{
  ...
  if ((IV != 3) && (IV != 4)) waste_line(KC,stream);
    ...
    (void)fscanf(stream,"%d %d %d %d %f %f %f",
    ...
    if (IV==3) (void)fscanf(stream,"%d %d %d",corner,corner+1,corner+2);
    else (void)fscanf(stream,"%d %d %d %d",corner,corner+1,corner+2,corner+3);
...
void grid_data(FILE *stream, double *trans_vector)
{
  ...
  (void)fscanf(stream, "%lf %lf %lf", coord, coord+1, coord+2);
...
void patch_data(FILE *stream)
{
  ...
  waste_line(9,stream);
  (void)fscanf(stream, "%lf %lf %lf %d %d %d %d", 
...
void CFEG_table(FILE *stream)
{
  ...
  int i, *element_list = 0, element_num1, element_num2;
  ...
  waste_line(1,stream);
  (void)fscanf(stream,"%d %d %d %d %d %d %d %d", 
...
void name_data(FILE *stream)
{
  ...
  (void)fgets(line, sizeof(line), stream); /* eat CR */
  (void)fgets(line, sizeof(line), stream);
  len = strlen(line);
  ...
  for(i = iv = 0; i < KC-1; i++) {      /* loop on lines */
    for(j = 0; j < 5 && iv < IV/2; j++, iv++) { /* loop on items */
      (void)fscanf(stream, "%d %d", &ntype, &id);
